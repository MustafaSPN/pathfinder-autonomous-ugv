# Pathfinder — RC Crawler

Ackermann-steered crawler, 0.313 m wheelbase, up to 46° of steering.
...
- **Drive** — Ackermann steering, non-holonomic
- **Positioning** — RTK GNSS at 20 Hz, dual-antenna heading
- **IMU** — BNO055 over UART
- **Wheel odometry** — **none**. There are no encoders on this vehicle. The local EKF is
  reduced to a heading estimator (gyro only, accelerometer deliberately disabled) and all
  position comes from RTK through the global filter
- **Nav2** — tuned for non-holonomic motion: reversing enabled, rotate-to-heading
  disabled, and recovery-free behavior trees because a car cannot spin in place
- **Extras on this branch** — `diag_gps.sh`, `diag_freeze.sh`, `verify_ekf.sh` field
  diagnostics
